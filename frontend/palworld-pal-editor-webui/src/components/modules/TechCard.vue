<template>
    <button type="button" :class="['tech', { bossTech: item.BossTechnology }, { locked: isLocked }]" :style="bgStyle"
        :disabled="palStore.LOADING_FLAG" :aria-pressed="!isLocked" @click="toggleLock">
        <div class="techHeader"><AppIcon v-if="!item.I18n.Type" name="warning" :size="13" /> {{ item.I18n.Type ?? "INVALID" }}</div>
