...
class PlayerEntity:
    ...
    @property
    def CommonItemContainerId(self) -> Optional[UUID]:
        return self.resolve_item_container_ids().get("CommonContainerId")

    def resolve_item_container_ids(self) -> dict[str, Optional[UUID]]:
        """Resolve player-owned inventory fields without exposing their raw paths."""
        upper_present = "InventoryInfo" in self._player_save_data
        lower_present = "inventoryInfo" in self._player_save_data
        if not upper_present and not lower_present:
            return {}
        upper = self._player_save_data.get("InventoryInfo")
        lower = self._player_save_data.get("inventoryInfo")
        if upper_present and lower_present and upper != lower:
            raise ValueError("InventoryInfo and inventoryInfo conflict")
        inventory_info = upper if upper_present else lower
        values = (inventory_info or {}).get("value", {})
        fields = (
            "CommonContainerId",
        ...
        return {
            field: PalObjects.get_PalContainerId(values.get(field))
            for field in fields
        }
    ...
    def save_new_pal_records(self):
        ...
        """
        for pal_entity in list(self._new_palbox.values()):
            self.record_new_pal(pal_entity)

        self._new_palbox.clear()

    def record_new_pal(self, pal_entity: PalEntity) -> None:
        ...
        if DataProvider.is_pal_invalid(pal_entity.DataAccessKey):
        ...
        elif pal_entity.IsHuman or not DataProvider.get_pal_sorting_key(
            pal_entity.DataAccessKey
        ):
            ...
            if DataProvider.get_tech_i18n(tech_key) is None:
                LOGGER.warning(
                    f"Technology {tech_key} not found, which may or may not be a bug. "
                    "If you are unsure please report to the dev."
                )
            else:
                self.toggle_UnlockedRecipeTechnologyNames(tech_key, True)
        pal_entity.is_new_pal = False
        self._new_palbox.pop(str(pal_entity.InstanceId), None)
