...
class PalEntity:
    ...
    def assign_owner(self, player) -> None:
        owner_id = player.PlayerUId
        if "OwnerPlayerUId" not in self._pal_param:
            self._pal_param["OwnerPlayerUId"] = PalObjects.Guid(owner_id)
        else:
            PalObjects.set_BaseType(self._pal_param["OwnerPlayerUId"], owner_id)
        if self.OldOwnerPlayerUIds is None:
        ...
        elif all(str(value) != str(owner_id) for value in self.OldOwnerPlayerUIds):
            self.OldOwnerPlayerUIds.append(owner_id)
        if "LastNickNameModifierPlayerUid" in self._pal_param:
            PalObjects.set_BaseType(
                self._pal_param["LastNickNameModifierPlayerUid"], owner_id
            )
        self.group_id = player.group_id
        self.set_owner_player_entity(player)

    def assign_clone_identity(
        self, instance_id: UUID | str, equip_item_container_id: UUID | str
    ) -> None:
        self.InstanceId = instance_id
        self.PlayerUId = PalObjects.EMPTY_UUID
        equip_item = self._pal_param.get("EquipItemContainerId")
        if equip_item is not None:
            if PalObjects.get_PalContainerId(equip_item) is None:
                raise ValueError("Unsupported EquipItemContainerId encoding")
            PalObjects.set_PalContainerId(equip_item, equip_item_container_id)
        self._pal_param.pop(
            "MapObjectConcreteInstanceIdAssignedToExpedition", None
        )
        self.is_unreferenced_pal = False
        self.is_new_pal = True
    ...
    def group_id(self, id: UUID | str):
        self._pal_obj["value"]["RawData"]["value"]["group_id"] = toUUID(id)
    ...
    def OwnerName(self) -> str:
        if self.owner_player_entity is not None:
            return self.owner_player_entity.NickName or self.OwnerPlayerUId
        from .save_manager import SaveManager

        manager = SaveManager()
        player = (
            manager.get_player(self.OwnerPlayerUId)
            if hasattr(manager, "player_mapping")
            else None
        )
    ...
    def set_WorkSuitability(self, suit: PalSuitability | str, rank: int) -> None:
        ...
        if rank <= 0:
            ...
            base_rank = suits[suit.value]
            condensation_bonus = int(
                (self.Rank or 0) >= 5 and rank <= 5 and base_rank < 5
            )
            added_rank = rank - condensation_bonus - base_rank
            if added_rank <= 0:
                PalObjects.pop_WorkSuitability(
                    self._pal_param["GotWorkSuitabilityAddRankList"], suit
                )
