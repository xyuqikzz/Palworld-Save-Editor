...
from palworld_pal_editor.config import PROGRAM_PATH, Config, version_info, is_gh_build, get_new_version, CONFIG_PATH, PROJECT_RELEASES_URL
...
def setup_config_from_args():
    ...
    parser = argparse.ArgumentParser(description="Palworld-Save-Editor")
...
def main():
    ...
    VER = version_info()
    LOGGER.info(f"Running Palworld-Save-Editor version: {VER}")
    if not is_gh_build():
