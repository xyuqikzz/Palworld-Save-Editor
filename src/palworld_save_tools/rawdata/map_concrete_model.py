...
def decode_bytes(
    ...
    if not reader.eof():
        # Concrete-model layouts gain fields between game versions.  This
        # editor does not mutate them, so retain any version tail exactly
        # instead of preventing ownership-only indexing of the world graph.
        data["trailing_unparsed_data"] = [int(b) for b in reader.read_to_end()]
    return data
