...
- Steam 与 Game Pass/WGS 账号级跨界帕鲁终端可在独立工作区中打开，新增、克隆、删除或编辑受支持的帕鲁与 NPC，包括佐伊以及佐伊与暴电熊。只要槽位结构合法，不会仅因 `CharacterID` 不在当前目录中而拒绝打开该记录。Steam 直接选择 `GlobalPalStorage.sav`；Game Pass/WGS 选择包含 `containers.index` 的用户目录并写回打开时的原账号槽位。保存使用来源冲突检测、WGS 外的已验证备份、事务替换和重新打开验证；修改结果尚未完成真实游戏加载与 Xbox 云同步验证。
...
Game Pass 使用前请完全退出 Palworld 并等待本地同步。桌面版系统选择器可选择目标 Xbox 用户目录或其 WGS 容器树中的子目录，程序会向上定位所属的 `containers.index`；Web/应用内浏览器还可选择 WGS 根目录、Xbox 用户目录或直接选择 `containers.index`。读取槽位后必须明确选择要打开的世界。本地事务成功以及待同步元数据均不代表 Xbox 云同步已经验证。

## 主要功能

- 打开只读世界总览，查看玩家、帕鲁、种类、据点、公会、远征、竞技场、健康状态和结构引用诊断。
- 优先使用可用的系统选择器浏览本地存档来源；回退到应用内浏览器时，可从文件系统根目录导航，并提供搜索、自然排序、修改时间、完整存档标记和离线文件类型图标。
- 浏览玩家、帕鲁、公会、据点、世界容器、物品和最后保存的地图位置。
- 修改玩家名称、等级、科技点、属性、任务和受支持的背包；可单独增减药剂与秘药带来的额外属性等级，并按各属性官方总等级上限校验。
- 修改物品数量和受支持的动态属性；复制、移动、替换或清空格子；在提示物品丢失风险后将已验证的普通背包缩小至官方基础容量 42 格，并扩展公会仓库容量（公会仓库不支持缩容）。
...
- 修改帕鲁种类、特殊形态、昵称、性别、信赖度、等级、个体值、浓缩、魂强化、工作适应性、主动技能、被动技能、自定义/模组被动条目和可复用预设。列表会识别高塔 BOSS、BOSS、克隆体、已觉醒、稀有、RAID、暴走、油田和远征状态。普通世界帕鲁在存在已验证的 `bImportedCharacter` 结构时可移除其 Global Palbox 克隆标记；此存档字段修改尚未在真实游戏中验证。魂强化保留存档等级显示，同时显示“等级 × 3%”的游戏加成；正常编辑上限为等级 20 / 60%，更高值仅在无限制模式提供。
- 新增、复制、删除和跨容器整理帕鲁；危险操作提供预览和引用检查；玩家所属帕鲁可按队伍、帕鲁箱和其他位置分组折叠，并按容器、图鉴或等级排序。
- 在独立会话中编辑 Steam 或 Game Pass/WGS 账号级跨界帕鲁终端，复用离线帕鲁管理界面修改属性、强化、工作适应性与技能；利用真实空位添加目录中的帕鲁或 NPC，使用佐伊、佐伊与暴电熊快捷预设，克隆或删除条目，并通过外部已验证备份安全保存到原来源。
- 修改受支持的公会名称，按职位查看成员，在已知结构上安全转移会长，在官方 1–35 级范围内修改帕鲁终端等级（确认降级前会提示终端帕鲁风险），扩展公会仓库容量，并查看据点、工作帕鲁、技能和健康状态。
...
- 通过 PalEditorBridge 先显示在线玩家，再在后台获取短期只读玩家快照，并按 `PlayerUId` 合并全部存档玩家；可筛选全部、在线和离线玩家，并按选中玩家懒加载资料、背包、科技、任务、属性、地图进度、队伍帕鲁和帕鲁箱。本机自动连接未发现游戏或桥接时，会显示聚焦且已本地化的处理说明。
...
- 查看绑定会话修订的待保存变更，并通过临时文件、验证、已验证备份、冲突检测、替换和重新打开检查进行显式保存；失败时保留恢复信息。Windows 下的存档暂存、Steam 备份、WGS 备份/提交/恢复事务和重新打开验证支持扩展长度路径；若路径仍超过平台限制，程序会在写入存档数据前停止并显示明确的处理建议。
- 界面支持 English、Français、日本語、한국어 和简体中文；应用内确认、输入、成功与错误弹窗均管理键盘焦点，并可按需展开技术详情。
