...
- Steam 与 Game Pass/WGS 账号级跨界帕鲁终端可在独立工作区中打开，新增、克隆、删除或编辑受支持的帕鲁与 NPC，包括佐伊以及佐伊与暴电熊。Steam 直接选择 `GlobalPalStorage.sav`；Game Pass/WGS 选择包含 `containers.index` 的用户目录并写回打开时的原账号槽位。保存使用来源冲突检测、WGS 外的已验证备份、事务替换和重新打开验证；修改结果尚未完成真实游戏加载与 Xbox 云同步验证。
