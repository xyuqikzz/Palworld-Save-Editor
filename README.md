...
- Account-level Global Palboxes from Steam and Game Pass/WGS can be opened in a separate workspace to add, clone, delete, or edit supported Pals and NPCs, including Zoe and Zoe & Grizzbolt. Steam opens `GlobalPalStorage.sav` directly; Game Pass/WGS opens the user directory containing `containers.index` and writes back only to the account slot that was opened. Saves use source conflict detection, a verified backup outside WGS, transactional replacement, and reopen verification; loading the result in the game and Xbox cloud synchronization have not been verified.
