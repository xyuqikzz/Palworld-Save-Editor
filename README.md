...
- Account-level Global Palboxes from Steam and Game Pass/WGS can be opened in a separate workspace to add, clone, delete, or edit supported Pals and NPCs, including Zoe and Zoe & Grizzbolt. Validly slotted records are not rejected solely because their `CharacterID` is absent from the current catalog. Steam opens `GlobalPalStorage.sav` directly; Game Pass/WGS opens the user directory containing `containers.index` and writes back only to the account slot that was opened. Saves use source conflict detection, a verified backup outside WGS, transactional replacement, and reopen verification; loading the result in the game and Xbox cloud synchronization have not been verified.
...
- Edit player names, levels, technology points, attributes, missions, and supported inventories, including Remedy and Elixir bonus ranks capped by each attribute's official combined total.
- Edit item quantities and supported dynamic attributes; copy, move, replace, or clear slots; resize verified ordinary backpacks down to the official 42-slot base capacity with an item-loss warning, and expand guild chests without shrinking them.
...
- Edit Pal species, variants, names, gender, trust, level, IVs, condensation, souls, work suitability, active skills, passive skills, custom/mod passive entries, and reusable presets. Lists identify Tower Boss, Boss, clone, Awakened, rare, raid, predator, oil-rig, and expedition states. A standard-world Pal's known Global Palbox clone marker can be removed when its verified `bImportedCharacter` structure is present; this save-field change has not been verified in a real game. Soul controls keep the stored rank visible, show its `rank × 3%` game bonus, and cap normal editing at rank 20 / 60%; higher values remain restricted to unrestricted mode.
...
- Edit an account-level Steam or Game Pass/WGS Global Palbox in its own session using the same Pal-management UI as offline saves for attributes, upgrades, work suitability, and skills; add catalog Pals or NPCs through real free slots, use presets for Zoe and Zoe & Grizzbolt, clone or delete entries, and save safely back to the original source through a verified external backup.
- Rename supported guilds, inspect role-sorted members, safely transfer Guild Master ownership on known layouts, edit Palbox levels within the official 1–35 range with a Palbox-Pal warning before confirming reductions, expand guild-chest capacities, and inspect bases, working Pals, skills, and conditions.
